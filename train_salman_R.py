...
def main():
    ...
    log(logfilename, 'GPUs:'+str(torch.cuda.device_count()))
...
def train(loader: DataLoader, model: torch.nn.Module, criterion, optimizer: Optimizer, epoch: int, noise_sd: float,
    ...
    for i, batch in enumerate(loader):
        ...
        for inputs, targets in mini_batches:
            # inputs, targets = (inputs.to(device)/255), targets.to(device)
            ...
            noise = torch.randn_like(inputs) * noise_sd
            ...
            targets = targets.unsqueeze(1).repeat(1, args.num_noise_vec).reshape(-1, 1).squeeze().to(device)
            outputs = model(noisy_inputs)
            loss = criterion(outputs, targets)

            acc1, acc5 = accuracy(outputs, targets, topk=(1, 1))
