...
from train_utils import AverageMeter, log, requires_grad_, test, accuracy
...
parser.add_argument('--arch', default='cifar_resnet56', type=str, choices=ARCHITECTURES)
...
parser.add_argument('--target_type', default=None, type=str,
                    help='target type of cost-matrix')
...
mode = f"smix_{args.alpha}_{args.num_steps}_m{args.mix_step}_epoch{args.epochs}"
if args.maxnorm_s:
...
if args.dataset == 'cifar10':
    if args.target_type == 'single':
        args.outdir = f"logs/{args.dataset}/smix_seed{args.seed_type}_noise{args.noise_sd}"
    else:
        args.outdir = f"logs/{args.dataset}/smix_lbd{args.lbd}_seed24"
elif args.dataset == 'imagenette':
    if args.target_type == 'single':
        args.outdir = f"logs/{args.dataset}/smixR_lbd{args.lbd}_seed7_noise{args.noise_sd}"
    else:
        args.outdir = f"logs/{args.dataset}/smixR_lbd{args.lbd}_seed37_noise{args.noise_sd}"

def main():
    train_loader, test_loader, criterion, model, optimizer, scheduler, \
    starting_epoch, logfilename, model_path, device, writer ,target_type = prologue(args)
    ...
    for epoch in tqdm(range(starting_epoch, args.epochs)):
        ...
        test_loss, test_acc = test(test_loader, model, criterion, epoch, args.noise_sd, device, writer, args.print_freq,args.n_classes)
        after = time.time()
...
def train(loader: DataLoader, model, optimizer: Optimizer, epoch: int, noise_sd: float,
    ...
    for i, batch in enumerate(loader):
        ...
        for inputs, targets in mini_batches:
            ...
            # pdb.set_trace()
            noises = [torch.randn_like(inputs) * noise_sd for _ in range(args.num_noise_vec)] # 4
            ...
            loss_xent = F.cross_entropy(logits_c, targets_c)
            # pdb.set_trace()
            if args.dataset=='cifar10':
                if args.target_type == 'single':
                    mask = (targets_c==3).int()
                elif args.target_type == 'multi':
                    mask = ((targets_c==2)|(targets_c==4)).int()
            elif args.dataset == 'imagenette':
                if args.target_type == 'single':
                    mask = (targets_c==7).int()
                elif args.target_type == 'multi':
                    mask = ((targets_c==3)|(targets_c==7)).int()
            elif args.dataset == 'ham':
                mask =  (targets==0).int()
            
            
            sen_logits = logits_c[mask]
            sen_targets = targets_c[mask]
            sen_loss = F.cross_entropy(sen_logits, sen_targets)

            loss_xent += args.lbd*sen_loss
            ...
            # sen_loss_mixup = F.kl_div(logits_mix_c[mask], targets_mix_c[mask], reduction='none').sum(1)
            # loss_mixup += args.lbd*sen_loss_mixup
            loss = loss_xent + args.eta * args.warmup_v * (ind_correct * loss_mixup).mean()
